...
class MapCNN(nn.Module):
    ...
    def __init__(
        ...
        sensor_list = list(observation_space.spaces.keys())

        #assert sensor_list == ['map', 'pointgoal'], print(sensor_list) # TODO update

        self._n_input_map = observation_space.spaces["map"].shape[2]
        ...
        cnn_dims = np.array(
                observation_space.spaces["map"].shape[:2], dtype=np.float32
            )
    ...
    def forward(self, observations):
        ...
        if self._n_input_map > 0:
            map_observations = observations["map"]
