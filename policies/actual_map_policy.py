...
class PointNavDRDNActualMapNet(Net):
    ...
    def __init__(self, observation_space, hidden_size):
        ...
        self.visual_encoder = MapCNN(observation_space, hidden_size)
    ...
    def forward(self, observations, rnn_hidden_states, prev_actions, masks):
        ...
        if True:#changed
            ...
            # #changed add
            #pdb.set_trace()
            #map_ = observations["map"]
            #map_ = torch.swapaxes(map_, 1, 3)
            #observations["map"] = map_
            perception_embed = self.visual_encoder(observations) ## encode back to policy
